      ...
      switchMap(
        response => {
          ...
          return this.dd.post("rel",entityName).pipe(
            switchMap(
              (rel: {[index:string]: any}) => {
                var obs: Observable<{ [index: string]: any; }> = new Observable;
                /**
                 * Se crea una instancia de obs que 
                 */
                var keys: string[] =  Object.keys(rel);
                for(var i:number = 0; i < keys.length; i++){
                  try{throw i} catch(j:any){
